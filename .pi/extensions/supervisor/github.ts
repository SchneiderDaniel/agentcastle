...
export function extractStructuredAuditOutput(output: string): StructuredAuditOutput | null {
	// Canonical marker: AUDIT_DECISION: APPROVED or AUDIT_DECISION: REJECTED
	const decisionMatch = output.match(/AUDIT_DECISION\s*:\s*(APPROVED|REJECTED)/g);
	// Fallback: standalone AUDIT_APPROVED or AUDIT_REJECTED (no colon)
	...
	if (decisionMatch && decisionMatch.length > 0) {
		// Canonical marker takes priority (last occurrence wins)
		...
		// Fallback to standalone AUDIT_APPROVED (last occurrence wins)
...
/** Build deterministic fallback comment from agent text output when COMMENT_BODY: marker is missing.
 *  Parses structured finding patterns (🔴 Critical, 🟡 Warning) for REJECTED,
 *  and AUDIT_SCORE + checklist items for APPROVED.
 *  Returns null if no usable content found. */
...
function buildRejectionCommentFallback(output: string): string | null {
	...
	// Parse finding blocks: **🔴 Critical — Dim: Title** followed by Symptom/Consequence/Remedy/Location
	// Handles both 🔴/🟡, Critical/Warning, various dash styles
	const findingPattern = /\*\*(🔴|🟡)\s*(Critical|Warning)\s*[—–-]\s*(.+?):\s*(.+?)\s*\*\*/g;
	let match: RegExpExecArray | null;
	const findings: Array<{ severity: string; label: string; dimTitle: string; location: string }> =
		[];

	// Collect all finding headers with their positions
	...
	if (headerPositions.length === 0) {
		// No structured findings found — try raw text for any section headers
		...
		if (hasRejectedHeader || hasCritical || hasWarning) {
			// Extract meaningful lines from output (skip audio markers, gh commands, markdown fences)
			...
			for (const l of rawLines) {
				...
				// Skip gh commands, file paths, debug lines
		...
		for (let i = 0; i < headerPositions.length; i++) {
			...
			// Extract symptom/consequence/remedy
...
function buildApprovalCommentFallback(output: string): string | null {
	...
	// Parse checklist items (- Dimension: ✓ or ❌)
	...
	// Try to extract summary section (text between "### Summary" and next heading or end)
