 ...
 *   - jonjonrankin/pi-caveman (multi-level, config, animation, autocomplete)
 *   - our approach (structured prompts, persistence instruction)
 *
 * Commands:
 *   /caveman [level]  Toggle caveman mode or set intensity
 *   /caveman off       Disable caveman mode (aliases: stop, quit)
 *   /caveman config    Open settings dialog
 *
 * Levels: off | lite | full | ultra | wenyan-lite | wenyan | wenyan-ultra | micro
 */
...
export default function caveman(pi: ExtensionAPI) {
    ...
    async function openConfig(ctx: ExtensionContext) {
        ...
        await ctx.ui.custom((_tui, theme, _kb, done) => {
            ...
            container.addChild(
                new Text(
                    theme.fg("dim", " Saved to ~/.pi/agent/caveman.json"),
                    0,
                    0,
                ),
            );
