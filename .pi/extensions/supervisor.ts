...
function getProjectFields(projectNumber: number, owner: string): ProjectField[] {
	const result = ghJson([
		"project",
		"field-list",
...
export function resolveExtensions(extensionsRaw: string | undefined): string[] {
	...
	if (!extensionsRaw || !extensionsRaw.trim()) {
		// No extensions specified — auto-inject context-info
	...
	if (extensions.length === 0) {
		// Only supervisor was listed — auto-inject context-info
		base.push("--extension", ".pi/extensions/context-info.ts");
		return base;
	}
	...
	// Auto-inject context-info if not already present
	...
	return new Promise((resolve) => {
		...
		const buildWidgetLines = (): string[] => {
			...
			if (contextInfoReceived && contextTokens !== undefined && contextWindow !== undefined) {
				statsParts.push(`Context: ${formatTokens(contextTokens)}/${formatTokens(contextWindow)}`);
			}
			...
			try {
				...
				switch (ev.type) {
					...
					case "tool_execution_start":
						...
						currentToolArgs = ev.args ? JSON.stringify(ev.args).slice(0, 200) : undefined;
						lastToolName = ev.toolName;
						const logArgs = ev.args ? JSON.stringify(ev.args).slice(0, 200) : "";
						fullLog.push(`🔧 ${ev.toolName}${logArgs ? ` ${logArgs}` : ""}`);
						scheduleFlush();
						break;
					...
					case "message_end": {
						...
						if (msg.role === "assistant") {
							...
							if (msg.usage) {
								tokenCount = msg.usage.totalTokens || msg.usage.input + msg.usage.output;
							}
							scheduleFlush();
							...
							if (resultText && resultText.trim()) {
								...
								fullLog.push(`📋 ${label}: ${lines[0]?.slice(0, 300) || "(no output)"}`);
								for (let i = 1; i < Math.min(lines.length, 6); i++) {
									if (lines[i].trim()) fullLog.push(`   ${lines[i].slice(0, 200)}`);
								}
							} else {
...
export default function supervisor(pi: ExtensionAPI) {
	...
	pi.registerMessageRenderer<SupervisorMessageDetails>(
		"supervisor",
		(message: any, _options: any, theme: any) => {
			...
			if (details.textOutput) {
				...
				for (const line of outputLines) {
					...
					c.addChild(new Text(fit(styledLine || " "), 1, 0));
				}
			}
		...
		handler: async (args: any, ctx: any) => {
			...
			try {
				...
				try {
					...
					if (msg.includes("missing required scopes") || msg.includes("project")) {
						ctx.ui.notify(
				...
				for (let i = 0; i < MAX_LOOPS; i++) {
					...
					const nextStatus = determineNextStatus(agentName, result.textOutput, loopStatus, config);
					if (!nextStatus) {
						ctx.ui.notify(
					...
					const nextOptId = findStatusOption(fields, statusField.id, nextStatus);
					if (!nextOptId) {
						ctx.ui.notify(`Cannot find '${nextStatus}' option on board.`, "warning");
						break;
					}

					try {
						setItemStatus(loopItem.id, projectId, statusField.id, nextOptId);
						ctx.ui.notify(`Issue #${issueNum} moved: ${loopStatus} → ${nextStatus}`, "info");
					} catch (err: any) {
