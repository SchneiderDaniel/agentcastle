...
describe("shSingleQuote (Issue 292)", () => {
	it("(D) wraps simple string in single quotes", () => {
		assert.equal(shSingleQuote("hello"), "'hello'");
	});

	it("(D) wraps empty string in single quotes", () => {
		assert.equal(shSingleQuote(""), "''");
	});

	it("(D) wraps path with spaces in single quotes", () => {
		assert.equal(shSingleQuote("/path/with spaces/file"), "'/path/with spaces/file'");
	});

	it("(D) wraps path with dollar sign in single quotes (prevents expansion)", () => {
		assert.equal(shSingleQuote("/path/with/$pecial/chars"), "'/path/with/$pecial/chars'");
	});

	it("(D) wraps path with backtick in single quotes (prevents command sub)", () => {
		assert.equal(
			shSingleQuote("/path/with/back" + "`" + "tick/file"),
			"'/path/with/back" + "`" + "tick/file'",
		);
	});

	it("(D) wraps path with double quote in single quotes", () => {
		assert.equal(shSingleQuote('/path/with/double"quote/file'), "'/path/with/double\"quote/file'");
	});

	it("(D) escapes embedded single quote in path", () => {
...
describe("ensureChromiumDeps — shell quoting (Issue 292)", () => {
	function createMockSetup(testCwd?: string) {
		const cwd = testCwd ?? fs.mkdtempSync(path.join(os.tmpdir(), "crawl4ai-test-"));
		const DEPS_DIR = path.join(cwd, ".pi", "chromium-deps");
		const DEPS_LIB_DIR = path.join(DEPS_DIR, "usr", "lib", "x86_64-linux-gnu");
		const bashCCmds: string[] = [];

		const exec = makeMockExec(
			async (cmd: string, args: string[], _opts?: { timeout?: number; signal?: AbortSignal }) => {
				if (cmd === "bash" && args[0] === "-c") bashCCmds.push(args[1]);
				const fullCmd = cmd + " " + args.join(" ");
				if (fullCmd.includes("test -f") && fullCmd.includes("libnspr4.so")) {
					return { code: 1, stdout: "", stderr: "" };
				}
		...
		const depsReady = new Map<string, { ready: boolean; timestamp: number; retries: number }>();
		return { cwd, exec, depsReady, DEPS_DIR, DEPS_LIB_DIR, bashCCmds };
	}

	it("(D) test -f uses quoted path", async () => {
		const { exec, depsReady, cwd, bashCCmds } = createMockSetup();
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		const testFCmd = bashCCmds.find((c) => c.startsWith("test -f"));
		assert.ok(testFCmd, "should have a test -f command");
		assert.match(testFCmd!, /test -f '.*libnspr4\.so'/);
	});

	it("(D) cd + apt-get download uses quoted DEPS_DIR", async () => {
		const { exec, depsReady, cwd, bashCCmds } = createMockSetup();
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		const cdCmd = bashCCmds.find((c) => c.includes("apt-get download"));
		assert.ok(cdCmd, "should have a cd + apt-get download command");
		assert.match(cdCmd!, /cd '.*chromium-deps' && apt-get download/);
	});

	it("(D) ls uses quoted DEPS_DIR", async () => {
		const { exec, depsReady, cwd, bashCCmds } = createMockSetup();
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		const lsCmd = bashCCmds.find((c) => c.includes("ls"));
		assert.ok(lsCmd, "should have a ls command");
		assert.match(lsCmd!, /ls '.*chromium-deps'\//);
		assert.ok(lsCmd!.includes("*.deb"), "should include glob pattern");
	});

	it("(D) pkg in apt-get download remains unquoted", async () => {
		const { exec, depsReady, cwd, bashCCmds } = createMockSetup();
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		const aptCmds = bashCCmds.filter((c) => c.includes("apt-get download"));
		assert.ok(aptCmds.length > 0, "should have apt-get download commands");
		for (const cmd of aptCmds) {
			const parts = cmd.split("apt-get download ");
			assert.ok(parts.length >= 2, "should have a package name");
			const pkg = parts[1].trim();
			assert.equal(pkg.startsWith("'"), false, "package name should not start with single quote");
			assert.equal(pkg.startsWith('"'), false, "package name should not start with double quote");
			assert.match(pkg, /^[a-z0-9]+$/, "package name should be alphanumeric");
		}
	});

	it("(D) cwd with spaces — all paths single-quoted", async () => {
		const cwd = "/tmp/my test project";
		const { exec, depsReady, bashCCmds } = createMockSetup(cwd);
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		assert.ok(bashCCmds.length > 0, "should have bash -c commands");
		// All bash -c commands should have the full space-containing path inside quotes
		for (const cmd of bashCCmds) {
			const quotedSections = cmd.match(/'[^']*'/g) || [];
			const hasDepsRef = quotedSections.some(
				(q) => q.includes(".pi") || q.includes("chromium-deps"),
			);
			if (hasDepsRef) {
				// The quoted section should span across the space in "my test"
				const matchingSection = quotedSections.find(
					(q) => q.includes(".pi") || q.includes("chromium-deps"),
				);
				assert.ok(
					matchingSection!.includes("my test project"),
					`quoted section should span past spaces: ${matchingSection}`,
				);
			}
		}
	});

	it("(D) cwd with shell-special chars — single quotes prevent expansion", async () => {
		const cwd = "/tmp/$HOME/back" + "`" + "tickdir";
		const { exec, depsReady, bashCCmds } = createMockSetup(cwd);
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		assert.ok(bashCCmds.length > 0, "should have bash -c commands");
		// Remove single-quoted sections; no bare $HOME or backtick should remain
		for (const cmd of bashCCmds) {
			const withoutQuotes = cmd.replace(/'[^']*'/g, "");
			assert.equal(
				withoutQuotes.includes("$HOME"),
				false,
				`$HOME should not appear outside quotes: ${cmd}`,
			);
			assert.equal(
				withoutQuotes.includes("`"),
				false,
				`backtick should not appear outside quotes: ${cmd}`,
			);
		}
	});

	it("(D) cwd with single quote — embedded quote properly escaped", async () => {
		const cwd = "/tmp/foo's dir";
		const { exec, depsReady, bashCCmds } = createMockSetup(cwd);
		await ensureChromiumDeps(exec, cwd, undefined, depsReady);

		assert.ok(bashCCmds.length > 0, "should have bash -c commands");
		// The path contains a single quote; shSingleQuote should escape it
		for (const cmd of bashCCmds) {
			if (cmd.includes("foo") && cmd.includes("s dir")) {
				assert.ok(cmd.includes("'\\''"), `embedded single quote should be escaped: ${cmd}`);
			}
		}
	});

	it("(D) all existing tests still pass with quoted bash -c args", async () => {
		// Re-run the full success path test from the retry cache suite
		// to ensure quoting changes don't break existing behavior
		const { cwd } = createMockSetup();
		const localDepsReady = new Map<
			string,
			{ ready: boolean; timestamp: number; retries: number }
		>();
		const localDEPS_DIR = path.join(cwd, ".pi", "chromium-deps");
		const localDEPS_LIB_DIR = path.join(localDEPS_DIR, "usr", "lib", "x86_64-linux-gnu");
		let testLibCallCount = 0;

		const localExec = makeMockExec(
			async (cmd: string, args: string[], _opts?: { timeout?: number; signal?: AbortSignal }) => {
				const fullCmd = cmd + " " + args.join(" ");
				if (fullCmd.includes("test -f") && fullCmd.includes("libnspr4.so")) {
					testLibCallCount++;
				...
				if (fullCmd.includes("mkdir") && fullCmd.includes("-p")) {
					fs.mkdirSync(localDEPS_DIR, { recursive: true });
					return { code: 0, stdout: "", stderr: "" };
				}
				if (fullCmd.includes("apt-get download")) {
					const pkgName = fullCmd.split("apt-get download ")[1];
					fs.writeFileSync(path.join(localDEPS_DIR, `${pkgName}.deb`), "fake");
					return { code: 0, stdout: "", stderr: "" };
				}
				if (fullCmd.includes("ls") && fullCmd.includes("*.deb")) {
					const debs = fs.readdirSync(localDEPS_DIR).filter((f) => f.endsWith(".deb"));
					return {
				...
				if (fullCmd.includes("dpkg") && fullCmd.includes("-x")) {
					fs.mkdirSync(localDEPS_LIB_DIR, { recursive: true });
					fs.writeFileSync(path.join(localDEPS_LIB_DIR, "libnspr4.so"), "");
					return { code: 0, stdout: "", stderr: "" };
				}
				return { code: 1, stdout: "", stderr: "mock: unhandled" };
			},
		...
		const result = await ensureChromiumDeps(localExec, cwd, undefined, localDepsReady);
		assert.equal(result, localDEPS_LIB_DIR, "should return DEPS_LIB_DIR on success");
	});
});
