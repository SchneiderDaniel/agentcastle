...
import { renderItemList } from "./widget-utils.ts";
...
export default function contextInfo(pi: ExtensionAPI): void {
	...
	pi.registerCommand("explain-prompts", {
		...
		handler: async (_args, ctx) => {
			...
			const items = prompts.map((pr) => ({
				name: pr.name,
				desc: (pr.description ?? "(no description)").split("\n")[0].trim(),
			}));

			ctx.ui.setWidget("explain-prompts", renderItemList(items, prompts.length, "prompts"));
		},
	});
	...
	pi.registerCommand("explain-skills", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("explain-skills", renderItemList(items, skills.length, "skills"));
		},
	});
	...
	pi.registerCommand("explain-extensions", {
		...
		handler: async (_args, ctx) => {
			...
			const items = extensions.map((ext) => ({
				...
				desc: ext.error
					? "error: " + ext.error
					: (ext.description ?? "(no description)").split("\n")[0].trim(),
			}));

			ctx.ui.setWidget("explain-extensions", renderItemList(items, extensions.length, "extensions"));
		},
	});
